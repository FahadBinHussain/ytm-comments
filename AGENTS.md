...
- debug: `parseCommentsPage` logs one-time `[ytm-comments] parse debug ...` lines (map sizes, sample thread keys, first surface/state payload preview). `onLike` logs `[ytm-comments] like blocked ...` with the full comment object. check these in the music.youtube.com tab console (page console, not the extension popup) when like misbehaves.
